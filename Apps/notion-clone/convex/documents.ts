...
export const archive = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const archiving = async (documnetId: Id<"documents">) => {
            const cildren = await ctx.db.query("documents").withIndex("by_user_parent", (q) => (
                q.eq("userId", userId).eq("parentDocument", documnetId)
            )).collect();

            for (const child of cildren) {
                await ctx.db.patch(child._id, {
                    isArchived: true,
                });

                await archiving(child._id);
            }
        }
...
export const restoreTrash = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (existingDocument.userId !== userId) {
            throw new Error("You are not authenticated for this document.");
        }
